import requests

recommendations_url = 'http://127.0.0.1:8000'
events_store_url = "http://127.0.0.1:8020"

headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}

# ----------------------- блендинг рекомендаций -------------------

user_id = 1000006
event_item_ids =  [53404, 33311009, 178529, 35505245, 795836] 
params = {"user_id": user_id, 'k': 5}

for event_item_id in event_item_ids:
    print(f'post to event store {event_item_id}')
    resp = requests.post(events_store_url + "/put", 
                         headers=headers, 
                         params={"user_id": user_id, "item_id": event_item_id})

print('Reccomendations offline ...')
resp_offline = requests.post(
    recommendations_url + "/recommendations_offline", headers=headers, params=params
    )
print('Reccomendations online ...')
resp_online = requests.post(
    recommendations_url + "/recommendations_online", headers=headers, params=params
    )
print('Reccomendations offline ...')
resp_blended = requests.post(
    recommendations_url + "/recommendations", headers=headers, params=params
    )

recs_offline = resp_offline.json()["recs"]
recs_online = resp_online.json()["recs"]
recs_blended = resp_blended.json()["recs"]

print('Reccomendations offline: ', recs_offline)
print('Reccomendations online: ', recs_online)
print('Recommendations blended: ', recs_blended)